...
```
♻︎ Entering the PAI ALGORITHM… (v3.9.1) ═════════════
...
- **PARALLELIZATION CHECK (MANDATORY for Extended+):** Before executing sequentially, identify independent workstreams:
  - Multiple URL fetches or API calls → batch with parallel `Agent` calls or concurrent `WebFetch`
  - Multiple file reads/explorations → parallel `Agent` with `run_in_background: true`
  - Independent code changes across files → consider `isolation: "worktree"` agents
  - Research from multiple sources → parallel research agents (the Research skill already does this)
  - If 3+ independent operations exist, parallelization is REQUIRED unless there are data dependencies between them.
...
- **Capability invocation audit (MANDATORY):** For EACH capability selected in OBSERVE, perform this structured check:
