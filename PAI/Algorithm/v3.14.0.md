...
### Mode Check (CRITICAL — before any ceremony)

**If your task produces FINDINGS rather than ARTIFACTS** (review, investigation, validation, research, debugging-to-diagnosis), you are in the WRONG mode. Exit this file immediately and use INVESTIGATE mode from CLAUDE.md. The Algorithm exists for building, not analyzing. A PR review does not need 18 ISC criteria and 7 phases.

**Smell test:** Will you commit code or produce a deliverable file at the end? If no → wrong mode.
...
- **EARLY EXIT CHECK:** If THINK reveals the task is actually investigation/research masquerading as a build (no code will be committed, no artifact produced), EXIT the Algorithm immediately. Report findings using INVESTIGATE mode format from CLAUDE.md. Don't force 5 more phases on a task that already has its answer.
