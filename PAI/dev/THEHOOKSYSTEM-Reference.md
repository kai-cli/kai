...
- Log errors to stderr (redirected to `/tmp/pai-hooks/` by run-hook.sh wrapper — Claude Code treats raw stderr as error)
...
### 7. **Logging**
- Log debug info to stderr (redirected to `/tmp/pai-hooks/*.log` by the run-hook.sh wrapper)
- **CRITICAL:** Claude Code treats stderr output from hooks as error — always register hooks via `run-hook.sh`, never directly
...
# Check hook logs (stderr redirected by run-hook.sh wrapper)
tail -f /tmp/pai-hooks/*.log       # All hook diagnostics
```
...
5. Hook performs actions (capture, notify, etc.)
