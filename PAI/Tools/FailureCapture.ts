...
export async function captureFailure(input: FailureCaptureInput): Promise<string | null> {
  ...
  // 1. Write sentiment.json (transcript not copied — original lives in ~/.claude/projects/)
  const sentimentData = {
    rating,
  ...
  // 3. Write tool-calls.json (structured + pre-parsed; auto-deleted after 90 days by SessionCleanup)
  writeFileSync(
...
|------|-------------|-----------|
| \`CONTEXT.md\` | This analysis document | Permanent |
| \`sentiment.json\` | Rating, summary, session ID | Permanent |
| \`tool-calls.json\` | Extracted tool invocations (${toolCalls.length} calls) | 90 days |

The raw session transcript is available at:
\`~/.claude/projects/.../sessions/${basename(transcriptPath)}\`

---

## Learning System Notes

This failure is actively injected into session context via \`loadFailurePatterns()\` in learning-readback.ts.
The CONTEXT.md summary is what gets read — the detail above is for manual review if needed.
`;
