...
import { runUpgrade, checkDependencies } from "./upgrade.ts";
...
async function cmdUpgrade(args: string[]) {
  ...
  for (let i = 0; i < args.length; i++) {
    if ((args[i] === '--source' || args[i] === '-s') && args[i + 1]) {
      ...
      dryRun = true;
    } else if (args[i] === '--skip-integrity') {
      skipIntegrity = true;
    } else if (args[i] === '--skip-deps') {
      skipDeps = true;
    } else if (args[i] === '--help' || args[i] === '-h') {
      console.log(`
pai upgrade — Upgrade PAI to a new release

USAGE:
  k upgrade --source <path>    Upgrade from a release directory
  k upgrade --dry-run          Show what would change without writing
  k upgrade --skip-integrity   Skip manifest checksum validation
  k upgrade --skip-deps        Skip Bun/Claude Code version checks
...
`);
  ...
  if (!source) {
    error('Usage: k upgrade --source <path-to-new-release/.claude>\n       k upgrade --help for full options');
  }
  ...
  if (result.errors.length > 0 && result.success) {
    // Warnings only
    for (const w of result.errors) log(w, '⚠️');
  }
  ...
  if (!result.success) {
    ...
    if (result.rolledBack) {
      log('Installation rolled back to previous version', '↩️');
    } else if (result.rolledBack === false && result.backupPath) {
      log(`Manual restore from backup: ${result.backupPath}`, '⚠️');
    }
    error(`Upgrade failed at phase: ${result.phase}`);
  }

  console.log();
  if (dryRun) {
    log(`Dry run complete — would backup to: ${result.backupPath}`, '✅');
  } else {
    log('PAI upgraded successfully', '✅');
    if (result.backupPath) {
      log(`Backup at: ${result.backupPath}`, 'ℹ️');
    }
    log('Restart Claude Code to apply changes', '⚠️');
  }
}
...
COMMANDS:
  ...
  k upgrade --source <p>   Upgrade PAI from a new release directory
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case "upgrade":
        command = "upgrade";
        // Collect remaining args for the upgrade sub-parser
        wallpaperArgs = args.slice(i + 1); // reuse wallpaperArgs slot for generic remainder
        i = args.length;
        break;
      case "profiles":
        command = "profiles";
  ...
  switch (command) {
    ...
    case "update":
      ...
      break;
    case "upgrade":
      await cmdUpgrade(wallpaperArgs);
      break;
    case "profiles":
      cmdProfiles();
      break;
