...
|--------|-----------|---------|
| **Structured Execution** | PAI Algorithm v3.10.0 | 7-phase problem-solving with ISC verification |
...
│
├── PAI/
│   ├── Algorithm/                 ← Algorithm spec (v3.10.0)
...
Current algorithm (v3.10.0) is spec-only — Claude reads it and follows it. Future: `algorithm.ts` enforces phase transitions programmatically, validates ISC format before proceeding, and prevents phase-skipping at the execution level (not just at the instruction level).
...
| Algorithm specification | `Releases/v4.4.0/.claude/PAI/Algorithm/v3.10.0.md` |
