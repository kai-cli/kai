...
Run these commands on `kai-cli/kai` BEFORE transferring to `kai-cli/kai`.

**Do not run on `kai-cli/pai-config` (personal repo) — that history stays private.**
...
```bash
git clone https://github.com/kai-cli/kai ~/kai-rewrite
cd ~/kai-rewrite
```
...
REDACTED-AWS-ACCOUNT==>REDACTED-AWS-ACCOUNT
your-aws-profile==>your-aws-profile
maintainer@kai-cli.com==>maintainer@kai-cli.com
EOF
)
```
...
git grep "REDACTED-AWS-ACCOUNT" $(git rev-list --all) 2>/dev/null | wc -l
# Should be 0

# Confirm old profile name gone
git grep "your-aws-profile" $(git rev-list --all) 2>/dev/null | wc -l
# Should be 0
```

## Step 5: Force-push to kai-cli/kai

```bash
git remote set-url origin https://github.com/kai-cli/kai.git
git push --force --all
git push --force --tags
```
...
4. Delete `kai-cli/kai` (GitHub will redirect for a period)
...
- After transfer, the personal `kai-cli/pai-config` is completely unaffected
