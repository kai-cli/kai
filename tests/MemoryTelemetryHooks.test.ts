...
describe('memory telemetry hook emitters', () => {
  ...
  test('memory telemetry report exposes agent return size pressure', () => {
    ...
    try {
      ...
      mkdirSync(stateDir, { recursive: true });
      writeFileSync(join(stateDir, 'memory-telemetry.jsonl'), [
        JSON.stringify({ ts: '2026-06-25T00:00:00.000Z', type: 'agent.return', project: 'Instant-Help', agent_type: 'general-purpose', description: 'Validate nav bugs', result_chars: 11070 }),
        JSON.stringify({ ts: '2026-06-25T00:00:01.000Z', type: 'agent.return', project: 'Instant-Help', agent_type: 'general-purpose', description: 'Validate naming bugs', result_chars: 10480 }),
        JSON.stringify({ ts: '2026-06-25T00:00:02.000Z', type: 'agent.checkpoint', project: 'Instant-Help', agent_type: 'general-purpose' }),
      ].join('\n') + '\n');
