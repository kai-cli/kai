#!/usr/bin/env bun
/**
 * transcript-parser.ts - Claude transcript parsing utilities (CANONICAL)
 *
 ...
 *
 * W13 (2026-06-05): Unified the two diverged copies that previously lived at
 * PAI/Tools/TranscriptParser.ts (field `completionSummary`) and
 * skills/PAI/Tools/TranscriptParser.ts (field `voiceCompletion`). Those paths
 * now re-export from here. Canonical field name is `completionSummary`.
 * NOTE: the `extractVoiceCompletion` FUNCTION keeps its name (consumed export);
 * only the ParsedTranscript FIELD was unified.
 *
 * CLI Usage:
